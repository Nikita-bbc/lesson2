...
async def get_buying_list(message):
    initiate_db()
    for num in range(len(get_all_products())):
        await message.answer(
            f"Название: {get_all_products()[num][1]} | Описание: {get_all_products()[num][2]}"
            f"| Цена: {get_all_products()[num][3]}")
        with open(f'photoes/img_{num + 1}.jpg', 'rb') as img:
